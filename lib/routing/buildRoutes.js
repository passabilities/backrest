...
let controllers = {}
_.each(glob.sync(`${process.cwd()}/${controllerPath}/*.js`), file => {
...
let buildRoute = (router, verb, url, to, root) => {
  ...
  const beforeFilters = getFilterMethods('before', controller, action)
  const afterFilters = getFilterMethods('after', controller, action)

  // Arguments to be passed in route initilizaion function.
  let args = [url, logger]
  // Add any before filters to the middleware stack.
  args = args.concat(beforeFilters)
  ...
  args.push((req, res) => {
    ...
    res.json = body => {
      ...
      _.each(afterFilters, afterFilter => {
        try {
  ...
  // Allow for OPTIONS request to check any headers that need to be set.
  router.options(url, beforeFilters)
